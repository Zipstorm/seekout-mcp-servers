...
1. **KISS (Keep It Simple)** - Choose the simplest solution that works. Complexity must be justified by concrete requirements.
2. **Single Responsibility (SRP)** - Each unit should have one reason to change. Separate concerns into focused components.
3. **Composition Over Inheritance** - Build behavior by combining objects, not extending classes.
4. **Rule of Three** - Wait until you have three instances before abstracting. Duplication is often better than premature abstraction.
